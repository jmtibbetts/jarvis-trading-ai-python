"""
Paper Trading Engine v1.1
Supports: Long, Long Leveraged, Short, Short Leveraged
Tracks a virtual account with P&L, mark-to-market, and margin simulation.

Fix v1.1:
- Corrected leveraged_pnl formula — raw_pnl already includes leverage via qty (no double-multiply)
- Corrected pnl_pct base — uses margin (cash at risk) not notional
- Added null-safety throughout trade_list serialization
- get_paper_summary is now crash-safe (skips bad records instead of raising)
"""
import logging
...
DEFAULT_POSITION_SIZE  = 2_000.0      # $2k margin per trade (2% of $100k)
...
def _calc_pnl(entry: float, close_price: float, qty: float, side: int, leverage: float, margin: float):
    ...
    """
    raw_pnl = (close_price - entry) * qty * side   # actual dollar gain/loss (leverage already in qty)
...
def open_paper_position(signal: dict, current_price: float = None) -> dict:
    ...
    target    = float(signal.get("target_price") or (entry * (1.05 if side == 1 else 0.95)))
    stop      = float(signal.get("stop_loss")    or (entry * (0.97 if side == 1 else 1.03)))
    margin    = DEFAULT_POSITION_SIZE              # cash at risk = $2k
    notional  = margin * leverage                  # total exposure
    ...
    with get_db() as db:
        ...
        pos_data = {
            ...
            "notional": notional, "margin_required": margin
        }

    logger.info(f"[Paper] Opened {dir_key} on {sym} @ ${entry:.4f} | notional=${notional:.0f} | margin=${margin:.0f}")
...
def close_paper_position(pos_id: str, close_price: float, reason: str = "manual") -> dict:
    ...
    with get_db() as db:
        ...
        # ── FIXED: raw_pnl already captures leverage through qty ──
        # qty = notional/entry = (margin * leverage) / entry
        # so raw_pnl = price_delta * qty * side = correct dollar P&L
        pnl, pnl_pct = _calc_pnl(entry, close_price, qty, side, lev, pos_margin)

        portfolio = _get_portfolio_cash(db)
        # Restore margin + actual P&L (pnl can be negative — correct cash deduction)
...
def get_paper_summary() -> dict:
    ...
    margin_in = sum(p["margin_used"] for p in pos_list)   # actual cash locked up
