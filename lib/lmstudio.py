...
v6.0: Supports LM Studio, Ollama, OpenAI, Anthropic, Groq, DeepSeek.
      Auto-detects provider from platform name and routes accordingly.
      Health check on startup. Streaming response logging.
"""
...
def get_llm_config() -> dict:
    """
    Read active LLM config from DB.
    Returns: {url, model, api_key, platform, max_tokens, provider}
    """
    try:
        with get_db() as db:
            ...
            # Priority: default LLM config first
...
def check_health() -> dict:
    ...
    try:
        ...
        return {'ok': False, 'platform': cfg['platform'], 'error': str(e)}
...
def call_lm_studio(prompt: str, system: str = None, max_tokens: int = None,
    ...
    """
    Unified LLM call — routes to correct provider automatically.
    """
    cfg = get_llm_config()
...
def _call_openai_compat(prompt: str, system: str, max_tokens: int,
    ...
    """OpenAI-compatible API call (LM Studio, Ollama, OpenAI, Groq, DeepSeek)."""
    messages = []
    if system:
    ...
    logger.debug(f"[LLM] POST {url} | model={cfg['model']} | max_tokens={max_tokens}")
    ...
    try:
        ...
        logger.debug(f"[LLM] Response: {tokens} tokens")
        return content
    except httpx.TimeoutException:
        raise RuntimeError(f"LLM timeout after {TIMEOUT}s — is {cfg['platform']} running?")
    except Exception as e:
        raise RuntimeError(f"LLM call failed ({cfg['platform']}): {e}")
...
def parse_json(text: str):
    ...
    logger.warning(f"[LLM] Could not parse JSON from response (len={len(text)}): {text[:200]}")
    return None
